...
class ControllerContentPost extends Controller {
    ...
    private function getList() {
        ...
        $template = ($this->config->get('default_admin_view_post_list')) ? $this->config->get('default_admin_view_post_list') : 'content/post_list.tpl';
    ...
    public function grid() {
        ...
        $post_total = $this->modelPost->getAllTotal($data);
        ...
        $pagination->url = Url::createAdminUrl('content/post/grid') . $url . '&page={page}';
        ...
        $template = ($this->config->get('default_admin_view_post_grid')) ? $this->config->get('default_admin_view_post_grid') : 'content/post_grid.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
    ...
    private function getForm() {
        ...
        if ($this->request->hasQuery('post_id') && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $post_info = $this->modelPost->getById($this->request->get['post_id']);
        }

        $this->setvar('post_id', $post_info, '');
        $this->setvar('parent_id', $post_info, '');
        $this->setvar('allow_reviews', $post_info, 1);
        $this->setvar('publish', $post_info, 1);
        ...
        $template = ($this->config->get('default_admin_view_post_form')) ? $this->config->get('default_admin_view_post_form') : 'content/post_form.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
    ...
    public function activate() {
        if (!isset($_GET['id']))
            return false;
        $this->load->auto('content/post');
        $status = $this->modelPost->getById($_GET['id']);
        if ($status) {
            if ($status['status'] == 0) {
