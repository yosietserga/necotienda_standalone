...
class ControllerEmailNewsletter extends Controller { 
    ...
    public function checkEmail() {
        ...
        if (isset($format) && $format == 'h') $email_size += $html_size;
        if (isset($format) && $format == 't') $email_size += $text_size;
        if (isset($format) && $format == 'a') $email_size = $html_size + $text_size;
        $email_size += 3000;
          ...
          foreach ($arrCheckSpam as $key => $spamrules) {
            if ($format == 'a' || $format == 'h') {
              if ($key == 'html') {
                ...
                if ($spamrules['score'] == 0) $html_output .= "<h3>No hay frases consideradas como Spam</h3>";
                        ...
                        jQuery(\"#btn_save_and_send\").click(function(){                    
                        	...
                        	jQuery.ajax({
                        		...
                        		beforeSend: function() {
                                  ...
                                  if (description.length < 1) {errormsg += 'Debe ingresar una descripci&oacute;n para la campaña\\n';}
