<?php
/**
 * ControllerModuleBestsellerWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleBestsellerWidget extends Controller {
	private $error = array(); 
	
	/**
	 * ControllerModuleBestsellerWidget::index()
	 * 
	 * @return
	 */
	public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings           = new stdClass;
                $settings->route    = 'module/bestseller';
                $settings->limit    = 4;
                $settings->autoload = 1;
                $data['settings']   = $settings;
                $widget->save($data);
    		...
    		$this->template = 'module/bestseller/widget.tpl';
