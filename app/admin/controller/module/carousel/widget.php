<?php
/**
 * ControllerModuleCarouselWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleCarouselWidget extends Controller {
	private $error = array(); 
	
	/**
	 * ControllerModuleCarouselWidget::index()
	 * 
	 * @return
	 */
	public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings           = new stdClass;
                $settings->route    = 'module/carousel';
                $settings->autoload = 1;
                $settings->width    = 150;
                $settings->height   = 150;
                $settings->scroll   = 1;
                $settings->limit    = 4;
                $settings->module   = 'latest';
                $data['settings']   = $settings;
                $widget->save($data);
                $this->setvar('name');
            }
    		...
    		$this->template = 'module/carousel/widget.tpl';
