<?php
/**
 * ControllerModuleCatalog2pdfWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleCatalog2pdfWidget extends Controller {
	private $error = array(); 
	
	/**
	 * ControllerModuleCatalog2pdfWidget::index()
	 * 
	 * @return
	 */
	public function index() {
		...
		$this->load->language('module/catalog2pdf');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings           = new stdClass;
                $settings->route    = 'module/catalog2pdf';
                $settings->autoload = 1;
                $data['settings']   = $settings;
                $widget->save($data);
                $this->setvar('name');
    		...
    		$this->template = 'module/catalog2pdf/widget.tpl';
