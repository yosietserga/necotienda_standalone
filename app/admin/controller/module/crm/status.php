 ...
 * @see Controller
 */
class ControllerModuleCrmStatus extends Controller {

    private $error = array();

    /**
     * ControllerModuleCrmStatus::index()
     * 
     * @return
     */
            ...
            foreach ($results as $result) {
                $action = array(
                    'edit' => array(
                        ...
                        'href' => Url::createAdminUrl('module/crm/status/update') . '&sale_status_id=' . $result['sale_status_id'] . $url,
                        'img' => 'edit.png'
                    ),
                    'delete' => array(
                        'action' => 'delete',
    ...
    public function create() {
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->request->getPost('to') === "saveAndKeep") {
                $this->redirect(Url::createAdminUrl('module/crm/status/update', array('sale_status_id' => $id)));
            } elseif ($this->request->getPost('to') === "saveAndNew") {
                $this->redirect(Url::createAdminUrl('module/crm/status/create'));
            } else {
                $this->redirect(Url::createAdminUrl('module/crm/status'));
            }
        }
    ...
    public function update() {
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->request->getPost('to') === "saveAndKeep") {
                $this->redirect(Url::createAdminUrl('module/crm/status/update', array('sale_status_id' => $this->request->getQuery('sale_status_id'))));
            } elseif ($this->request->getPost('to') === "saveAndNew") {
                $this->redirect(Url::createAdminUrl('module/crm/status/create'));
            } else {
                $this->redirect(Url::createAdminUrl('module/crm/status'));
    ...
    public function grid() {
        ...
        $pagination->limit = $limit;
        $pagination->text = $this->language->get('text_pagination');
        $pagination->url = Url::createAdminUrl('module/crm/status/grid') . $url . '&page={page}';
    ...
    protected function getForm() {
        ...
        $this->document->breadcrumbs[] = array(
            'href' => Url::createAdminUrl('module/crm/status') . $url,
            'text' => $this->language->get('heading_title'),
            'separator' => ' :: '
        );

        if (!$this->request->hasQuery('sale_status_id')) {
            $this->data['action'] = Url::createAdminUrl('module/crm/status/create') . $url;
        } else {
            $this->data['action'] = Url::createAdminUrl('module/crm/status/update') . '&sale_status_id=' . $this->request->getQuery('sale_status_id') . $url;
        }

        $this->data['cancel'] = Url::createAdminUrl('module/crm/status') . $url;
    ...
    /**
     * ControllerModuleCrmStatus::validate()
     * 
     * @return
     */
