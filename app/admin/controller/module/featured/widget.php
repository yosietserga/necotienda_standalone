<?php
/**
 * ControllerModuleFeaturedWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleFeaturedWidget extends Controller {
	private $error = array(); 
	
	/**
	 * ControllerModuleFeaturedWidget::index()
	 * 
	 * @return
	 */
	public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings           = new stdClass;
                $settings->route    = 'module/featured';
                $settings->autoload = 1;
                $settings->limit    = 4;
                $data['settings']   = $settings;
                $widget->save($data);
    		...
    		$this->template = 'module/featured/widget.tpl';
