<?php
/**
 * ControllerModuleGoogleMapsWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleGoogleMapsWidget extends Controller {
	private $error = array(); 
	
	/**
	 * ControllerModuleGoogleMapsWidget::index()
	 * 
	 * @return
	 */
	public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings           = new stdClass;
                $settings->route    = 'module/google_maps';
                $settings->autoload = 1;
                $data['settings']   = $settings;
                $widget->save($data);
                $this->setvar('name');
    		...
    		$this->template = 'module/google_maps/widget.tpl';
