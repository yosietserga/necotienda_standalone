<?php
/**
 * ControllerModuleLayerSliderWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleLayerSliderWidget extends Controller {
	private $error = array(); 
	
	/**
	 * ControllerModuleLayerSliderWidget::index()
	 * 
	 * @return
	 */
	public function index() {
		...
		$this->load->model('content/layer_slider');
        $this->data['layer_sliders'] = $this->modelLayerSlider->getAll();
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings           = new stdClass;
                $settings->route    = 'module/layer_slider';
                $settings->autoload = 1;
                $data['settings']   = $settings;
                $widget->save($data);
                $this->setvar('name');
    		...
    		$this->template = 'module/layer_slider/widget.tpl';
