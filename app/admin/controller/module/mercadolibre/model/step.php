...
class ModelStep extends Model {
    ...
    public function add($data) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "crm_sale_step SET 
        ...
        $this->cache->delete('crm_sale_step');
        return $id;
    }
    ...
    public function update($id, $data) {
        $this->db->query("UPDATE " . DB_PREFIX . "crm_sale_step SET 
        language_id = '". (int)$this->config->get('config_language_id') ."',
        name = '" . $this->db->escape($data['name']) . "', 
        properties = '" . serialize($data['properties']) . "', 
        date_modified = NOW() 
        WHERE sale_step_id = '" . (int) $id . "'");
        $this->cache->delete('crm_sale_step');
    }
    ...
    public function delete($id) {
        $this->db->query("DELETE FROM " . DB_PREFIX . "crm_sale_step WHERE sale_step_id = '" . (int) $id . "'");
        $this->db->query("UPDATE " . DB_PREFIX . "crm_sale_opportunity SET sale_step_id = '0' WHERE sale_step_id = '" . (int) $id . "'");
        $this->cache->delete('crm_sale_step');
    }
    ...
    public function getById($id) {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "crm_sale_step WHERE sale_step_id = '" . (int) $id . "'");
    ...
    public function getAll($data = array()) {
        if ($data) {
            $sql = "SELECT * FROM " . DB_PREFIX . "crm_sale_step s";
            ...
            $cache_data = $this->cache->get('admin.crm_sale_step.all');

            if (!$cache_data) {
                $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "crm_sale_step ORDER BY sort_order");

                $cache_data = $query->rows;

                $this->cache->set('admin.crm_sale_step.all', $cache_data);
            }
    ...
    public function getAllTotal($data = array()) {
        $sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "crm_sale_step s";
    ...
    public function sort($data) {
        ...
        $pos = 1;
        foreach ($data as $id) {
            $this->db->query("UPDATE " . DB_PREFIX . "crm_sale_step SET sort_order = '" . (int) $pos . "' WHERE sale_step_id = '" . (int) $id . "'");
            $pos++;
        }
        return true;
    }
    ...
    /**
     * ModelStep::getProperty()
     * 
     * Obtener una propiedad de la categoria
     * 
     * @param int $id sale_step_id
     * @param varchar $group
     * @param varchar $key
     * @return mixed value of property
     * */
    public function getProperty($id, $group, $key) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "crm_sale_step_property 
        WHERE `sale_step_id` = '" . (int) $id . "' 
    ...
    public function setProperty($id, $group, $key, $value) {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "crm_sale_step_property SET
        `sale_step_id`   = '" . (int) $id . "',
    ...
    public function deleteProperty($id, $group, $key) {
        $this->db->query("DELETE FROM " . DB_PREFIX . "crm_sale_step_property 
        WHERE `sale_step_id` = '" . (int) $id . "' 
        AND `group` = '" . $this->db->escape($group) . "'
        AND `key` = '" . $this->db->escape($key) . "'");
    }
