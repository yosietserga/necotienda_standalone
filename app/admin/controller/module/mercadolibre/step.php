...
class ControllerModuleCrmStep extends Controller {
            ...
            foreach ($results as $result) {
                $action = array(
                    'edit' => array(
                        ...
                        'href' => Url::createAdminUrl('module/mercadolibre/step/update') . '&sale_step_id=' . $result['sale_step_id'] . $url,
                        'img' => 'edit.png'
                    ),
                    'delete' => array(
                        'action' => 'delete',
    ...
    public function create() {
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->request->getPost('to') === "saveAndKeep") {
                $this->redirect(Url::createAdminUrl('module/mercadolibre/step/update', array('sale_step_id' => $id)));
            } elseif ($this->request->getPost('to') === "saveAndNew") {
                $this->redirect(Url::createAdminUrl('module/mercadolibre/step/create'));
            } else {
                $this->redirect(Url::createAdminUrl('module/mercadolibre/step'));
            }
        }
    ...
    public function update() {
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->request->getPost('to') === "saveAndKeep") {
                $this->redirect(Url::createAdminUrl('module/mercadolibre/step/update', array('sale_step_id' => $this->request->getQuery('sale_step_id'))));
            } elseif ($this->request->getPost('to') === "saveAndNew") {
                $this->redirect(Url::createAdminUrl('module/mercadolibre/step/create'));
            } else {
                $this->redirect(Url::createAdminUrl('module/mercadolibre/step'));
    ...
    public function grid() {
        ...
        $pagination->limit = $limit;
        $pagination->text = $this->language->get('text_pagination');
        $pagination->url = Url::createAdminUrl('module/mercadolibre/step/grid') . $url . '&page={page}';
    ...
    protected function getForm() {
        ...
        $this->document->breadcrumbs[] = array(
            'href' => Url::createAdminUrl('module/mercadolibre/step') . $url,
            'text' => $this->language->get('heading_title'),
            'separator' => ' :: '
        );

        if (!$this->request->hasQuery('sale_step_id')) {
            $this->data['action'] = Url::createAdminUrl('module/mercadolibre/step/create') . $url;
        } else {
            $this->data['action'] = Url::createAdminUrl('module/mercadolibre/step/update') . '&sale_step_id=' . $this->request->getQuery('sale_step_id') . $url;
        }

        $this->data['cancel'] = Url::createAdminUrl('module/mercadolibre/step') . $url;
