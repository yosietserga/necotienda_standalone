...
/**
 * ControllerModuleMercadoLibreWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleMercadoLibreWidget extends Controller {

    private $error = array();

    /**
     * ControllerModuleMercadoLibreWidget::index()
     * 
     * @return
     */
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings = new stdClass;
                $settings->route = 'module/mercadolibre';
                $settings->autoload = 1;
                $data['settings'] = $settings;
                $widget->save($data);
                $this->setvar('name');
            ...
            $this->template = 'module/mercadolibre/widget.tpl';
