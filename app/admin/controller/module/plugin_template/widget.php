<?php
/**
 * ControllerModulePluginTemplateWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModulePluginTemplateWidget extends Controller {
	private $error = array(); 
	
	/**
	 * ControllerModulePluginTemplateWidget::index()
	 * 
	 * @return
	 */
	public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings           = new stdClass;
                $settings->route    = 'module/plugin_template';
                $settings->autoload = 1;
                $data['settings']   = $settings;
                $widget->save($data);
                $this->setvar('name');
    		...
    		$this->template = 'module/plugin_template/widget.tpl';
