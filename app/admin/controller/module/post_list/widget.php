...
/**
 * ControllerModulePostListWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModulePostListWidget extends Controller {

    private $error = array();
    private $module = 'post_list';

    /**
     * ControllerModulePostListWidget::index()
     * 
     * @return
     */
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $c = $this->modelPost_category->getAllForList(0, null);
            ...
            $template = ($this->config->get('default_admin_view_module_post_list_widget')) ? $this->config->get('default_admin_view_module_post_list_widget') : 'module/'. $this->module .'/widget.tpl';
        ...
        if ($categories) {
            foreach ($categories as $result) {
                if ($parent === true)
                    ...
                    $output .= '<li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                $output .= '<input id="'. $name .'Settingscategories' . $result['post_category_id'] . '" type="checkbox" name="Widgets['. $name .'][settings][categories][]" value="' . $result['post_category_id'] .'"';
                $output .= (in_array($result['post_category_id'], $settingsCategories) || empty($settingsCategories)) ? ' checked="checked"' : '';
                $output .= '">';
                $output .= '<label for="'. $name .'Settingscategories' . $result['post_category_id'] . '">' . $result['name'] . '</label>';
