...
/**
 * ControllerModuleProductTabsWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleProductTabsWidget extends Controller {

    private $error = array();
    private $module = 'product_tabs';

    /**
     * ControllerModuleProductTabsWidget::index()
     * 
     * @return
     */
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $m = $this->modelManufacturer->getAll();
            ...
            if ($this->request->hasQuery('w')) {
                ...
                foreach ($this->data['settings']['tabs'] as $k => $tab) {
                    $t = array();
                    $t['categories'] = $this->getCategories($k, $c, true, $widget_info['name'], $tab['categories']);
                    $t['manufacturers'] = $this->getManufacturers($k, $m, $widget_info['name'], $tab['manufacturers']);
                    $t['module'] = $tab['module'];
                    $t['view'] = $tab['view'];
                ...
                foreach($settings->tabs as $k => $tab) {
                    $tab = array();
                    $tab['categories'] = $this->getCategories($k, $c, true, $data['name'], null);
                    $tab['manufacturers'] = $this->getManufacturers($k, $m, $data['name'], null);
                    $tab['module'] = 'random';
                    $tab['view'] = 'carousel';
                    $settings->tabs[$k] = $tab;
                }
            ...
            $template = ($this->config->get('default_admin_view_module_product_tabs_widget')) ? $this->config->get('default_admin_view_module_product_tabs_widget') : 'module/'. $this->module .'/widget.tpl';
    ...
    private function getCategories($index, $categories, $parent = false, $name, $settingsCategories) {
        $output = '';
        if ($categories) {
            foreach ($categories as $result) {
                ...
                // subcategories
                if ($result['childrens']) {
                    $output .= $this->getCategories($index, $result['childrens'], false, $name, $settingsCategories);
                }
                $output .= '</li>';
            }
        }
    ...
    private function getManufacturers($index, $manufacturers, $name, $settingsManufacturers) {
        $output = '';
        if ($manufacturers) {
