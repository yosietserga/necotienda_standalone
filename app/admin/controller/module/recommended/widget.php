<?php
/**
 * ControllerModuleRecommendedWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleRecommendedWidget extends Controller {
	private $error = array(); 
	
	/**
	 * ControllerModuleRecommendedWidget::index()
	 * 
	 * @return
	 */
	public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings           = new stdClass;
                $settings->route    = 'module/recommended';
                $settings->autoload = 1;
                $settings->limit    = 4;
                $data['settings']   = $settings;
                $widget->save($data);
    		...
    		$this->template = 'module/recommended/widget.tpl';
