<?php
/**
 * ControllerModuleSubscribeWidget
 * 
 * @package NecoTienda
 * @author Yosiet Serga
 ...
 * @see Controller
 */
class ControllerModuleSubscribeWidget extends Controller {
	private $error = array(); 
	
	/**
	 * ControllerModuleSubscribeWidget::index()
	 * 
	 * @return
	 */
	public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->request->hasQuery('w')) {
                ...
                $settings           = new stdClass;
                $settings->route    = 'module/subscribe';
                $settings->autoload = 1;
                $data['settings']   = $settings;
                $widget->save($data);
                $this->setvar('name');
    		...
    		$this->template = 'module/subscribe/widget.tpl';
