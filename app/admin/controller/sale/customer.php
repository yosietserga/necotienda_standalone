...
class ControllerSaleCustomer extends Controller {
    ...
    public function insert() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $customer_id = $this->modelCustomer->add($this->request->post);

            foreach ($this->request->post['addresses'] as $address) {
                $address['customer_id'] = $customer_id;
                $this->modelAddress->add($address);
            }
    ...
    public function update() {
        $this->document->title = $this->language->get('heading_title');
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->modelCustomer->update($this->request->getQuery('customer_id'), $this->request->post);

            $this->modelAddress->deleteByCustomerId($this->request->getQuery('customer_id'));
            foreach ($this->request->post['addresses'] as $address) {
                $address['customer_id'] = $this->request->getQuery('customer_id');
                $this->modelAddress->add($address);
            }
    ...
    private function getList() {
        ...
        $template = ($this->config->get('default_admin_view_sale_customer_list')) ? $this->config->get('default_admin_view_sale_customer_list') : 'sale/customer_list.tpl';
    ...
    public function grid() {
        ...
        $data = array(
            'name' => $filter_name,
            'email' => $filter_email,
            'customer_group_id' => $filter_customer_group_id,
        ...
        foreach ($results as $result) {
            ...
            $action = array(
                ...
                'approve' => array(
                    ...
                    'href' => '',
                    'img' => ($result['approved'] == 1) ? $i . 'customer_unlocked.png' : $i . 'customer_locked.png'
                ),
                'edit' => array(
                    'action' => 'edit',
        ...
        $template = ($this->config->get('default_admin_view_sale_customer_grid')) ? $this->config->get('default_admin_view_sale_customer_grid') : 'sale/customer_grid.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
    ...
    private function getForm() {
        ...
        if (isset($this->request->get['customer_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $customer_info = $this->modelCustomer->getById($this->request->get['customer_id']);
        }
        ...
        if (isset($this->request->post['addresses'])) {
            ...
            $this->data['addresses'] = $this->modelAddress->getAll(array(
        ...
        $template = ($this->config->get('default_admin_view_sale_customer_form')) ? $this->config->get('default_admin_view_sale_customer_form') : 'sale/customer_form.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
    ...
    public function see() {
        ...
        $template = ($this->config->get('default_admin_view_sale_customer_see')) ? $this->config->get('default_admin_view_sale_customer_see') : 'sale/customer_see.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
        ...
        if (!$this->user->hasPermission('modify', 'sale/customer')) {
            ...
            if (isset($this->request->post['selected'])) {
                foreach ($this->request->post['selected'] as $customer_id) {
                    $customer_info = $this->modelCustomer->getById($customer_id);
    ...
    public function activate() {
        if (!isset($_GET['id']))
            return false;
        $this->load->auto('sale/customer');
        $status = $this->modelCustomer->getById($_GET['id']);
        if ($status) {
            if ($status['status'] == 0) {
