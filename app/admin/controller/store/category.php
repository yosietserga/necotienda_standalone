...
class ControllerStoreCategory extends Controller {
    ...
    private function getList() {
        ...
        $template = ($this->config->get('default_admin_view_store_category_list')) ? $this->config->get('default_admin_view_store_category_list') : 'store/category_list.tpl';
    ...
    public function grid() {
        ...
        $filter_name = !empty($this->request->get['title']) ? $this->request->get['title'] : null;
        $filter_product = !empty($this->request->get['product']) ? $this->request->get['product'] : null;
        $filter_date_start = !empty($this->request->get['date_start']) ? $this->request->get['date_start'] : null;
        $filter_date_end = !empty($this->request->get['date_end']) ? $this->request->get['date_end'] : null;
        $page = !empty($this->request->get['page']) ? $this->request->get['page'] : 1;
        $sort = !empty($this->request->get['sort']) ? $this->request->get['sort'] : 'c.sort_order';
        ...
        $data = array(
            ...
            'date_start' => ($filter_date_start) ? date('Y-m-d h:i:s', strtotime($start[2] . "-" . $start[1] . "-" . $start[0])) : null,
            'date_end' => ($filter_date_end) ? date('Y-m-d h:i:s', strtotime($end[2] . "-" . $end[1] . "-" . $end[0])) : null,
            'sort' => $sort,
            'order' => $order,
            'start' => ($page - 1) * $limit,
            'limit' => $limit
        ...
        $d = $data;
        $d['parent_id'] = 0;
        $this->data['categories'] = $this->getCategories($this->modelCategory->getAll($d), true);
        ...
        $template = ($this->config->get('default_admin_view_store_category_grid')) ? $this->config->get('default_admin_view_store_category_grid') : 'store/category_grid.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
    ...
    private function getForm() {
        ...
        $template = ($this->config->get('default_admin_view_store_category_form')) ? $this->config->get('default_admin_view_store_category_form') : 'store/category_form.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
    ...
    public function attributes() {
        ...
        $data['results'] = $this->modelAttribute->getAll(array(
