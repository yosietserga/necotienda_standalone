...
class ControllerStoreProduct extends Controller {
    ...
    public function lista() {
        ...
        $template = ($this->config->get('default_admin_view_store_product_lista')) ? $this->config->get('default_admin_view_store_product_lista') : 'store/product_lista.tpl';
     ...
     * @return void
     */
    public function insertWizard() {
        /*
        - Select Product Type
        - Select Categories, Brand
        - Add photos, details, price and attributes
         ...
         *  - add anchor to uri with the code of the product type
         *  - send a http request with the product type selected
         *  - generate empty product with a ramdon generated string as model
         *  - save product
         *  - load general form
         *  - load product type form
         *  - show relations form
         *  - when NEXT:
         *    - validate data
         *    - update product
         ...
         *            - validate data
         *            - update product
         *            - show successfully product saved and published
         *            - show products related and promotion form
         *
         *
         */

        $this->data['product_id'] = $this->request->getQuery('product_id');
        $step = trim(strtolower($this->request->getQuery('step')));

        $this->data['product_types'] = array(
            'si'=>array(
                'name'=>$this->language->get('Single'),
                'type'=>'single',
                'icon'=>'icons/product-types/product-type-single.png'
            ),
            'di'=>array(
                'name'=>$this->language->get('Digital'),
                'type'=>'digital',
                'icon'=>'icons/product-types/product-type-digital.png'
            ),
            'co'=>array(
                'name'=>$this->language->get('Combo'),
                'type'=>'combo',
                'icon'=>'icons/product-types/product-type-combo.png'
            ),
            'se'=>array(
                'name'=>$this->language->get('Service'),
                'type'=>'service',
                'icon'=>'icons/product-types/product-type-service.png'
            ),
            'gb'=>array(
                'name'=>$this->language->get('Group Buying'),
                'type'=>'group-buying',
                'icon'=>'icons/product-types/product-type-group-buying.png'
            ),
            'au'=>array(
                'name'=>$this->language->get('Auction'),
                'type'=>'auction',
                'icon'=>'icons/product-types/product-type-auction.png'
            )
        );
        ...
        switch($step) {
            ...
            case 'relations':
                 ...
                 *  - load general form
                 *  - load product type form
                 *  - show relations form (categories, manufacturers, customer groups, stores, etc.)
                 */
                break;

            case 'images':
                /*
                 *  - if not post data, error
                 *  - validate post data for categories
                 *  - if everything it's ok
                     *    - update product
                     *    - show images form
                 *  - if there are something wrong, return error message
                 */
                break;
            ...
            case 'descriptions':
                /*
                 *  - if not post data, error
                 *  - validate post data for images
                 *  - validate file processing
                 *  - if everything it's ok
                     *    - update product
                     *    - save images
                     *    - show descriptions form
                 *  - if there are something wrong, return error message
                 */
                break;
            ...
            case 'attributes':
                /*
                 *  - if not post data, error
                 *  - validate post data for descriptions
                 *  - validate image processing from description
                 *  - if everything it's ok
                     *    - update product
                     *    - save images
                     *    - show product type custom form and categories custom attributes inputs
                 *  - if there are something wrong, return error message
                 */
                break;
            ...
            case 'promotion':
                /*
                 *  - if not post data, error
                 *  - validate post data for attributes and product type custom data
                 *  - if everything it's ok
                     *    - update product
                     *    - show successfully product saved and published
                     *    - show products related and promotion form
        ...
        $template = ($this->config->get('default_admin_view_store_product_wizard_form')) ? $this->config->get('default_admin_view_store_product_wizard_form') : 'store/product_wizard_form.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
    ...
    public function producttypeform() {
        $product_type = $this->request->hasQuery('type') ? $this->request->getQuery('type') : 'single';
        $product_id = $this->request->hasQuery('product_id') ? $this->request->getQuery('product_id') : null;

        if ($product_id) {
            //load form to update the product
        } else {
            //create an empty product of this type
        }
        //return product form for this type
    }
    ...
    public function see() {
        ...
        $template = ($this->config->get('default_admin_view_store_product_see')) ? $this->config->get('default_admin_view_store_product_see') : 'store/product_see.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
    ...
    private function getList() {
        ...
        $template = ($this->config->get('default_admin_view_store_product_list')) ? $this->config->get('default_admin_view_store_product_list') : 'store/product_list.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
    ...
    public function grid() {
        ...
        $template = ($this->config->get('default_admin_view_store_product_grid')) ? $this->config->get('default_admin_view_store_product_grid') : 'store/product_grid.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
    ...
    private function getForm() {
        ...
        $this->data['error_title'] = ($this->error['title']) ? $this->error['title'] : '';
        ...
        if (!$this->request->getQuery('product_id')) {
            ...
            $this->data['action'] = Url::createAdminUrl('store/product/update') . '&product_id=' . $this->request->getQuery('product_id') . $url;
        }

        $this->data['cancel'] = Url::createAdminUrl('store/product') . $url;

        if ($this->request->getQuery('product_id') && $this->request->server['REQUEST_METHOD'] != 'POST') {
            $product_info = $this->modelProduct->getById($this->request->getQuery('product_id'));
        }
        ...
        foreach ($attrValues as $k => $attr) {
            ...
            $this->data['attributes'][$k] = $rows[0];
            $this->data['attributes'][$k]['categoriesAttributes'] = array_unique($this->modelProduct->getCategoriesByAttributeGroupId($k));
            foreach ($this->data['attributes'][$k]['attributes'] as $j => $attribute) {
                $this->data['attributes'][$k]['attributes'][$j]['value'] = $attr[$attribute['product_attribute_id']];
            }
        }
        /* /product attributes */
        ...
        $template = ($this->config->get('default_admin_view_store_product_form')) ? $this->config->get('default_admin_view_store_product_form') : 'store/product_form.tpl';
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_admin_template') . '/'. $template)) {
            $this->template = $this->config->get('config_admin_template') . '/' . $template;
        } else {
            $this->template = 'default/' . $template;
