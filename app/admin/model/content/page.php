...
class ModelContentPage extends Model {
    ...
    /**
     * @var $post_id | integer[11] | ID
     */
    protected $post_id = 0;

    /**
     * @var $post_type | varchar[255]
     */
    protected $post_type = '';
    ...
    /**
     * @var $date_publish_start | datetime
     */
    protected $date_publish_start = '';

    /**
     * @var $date_publish_end | datetime
     */
    protected $date_publish_end = '';

    /**
     * @var $publish | integer[1]
     */
    protected $publish = 0;

    /**
     * @var $allow_reviews | integer[11]
     */
    protected $allow_reviews = 0;

    /**
     * @var $attributes cotenedor del mapping de los atributos
     */
    protected $attributes = array();

    /**
     * @var $error contenedor de los mensajes de error
     */
    protected $error;

    public function init() {
        $this->__mapAttributes();
    }

    public function formValidate($data = null) {
        if (!$data) {
            foreach ($this->attributes as $attribute => $rules) {
                if (isset($rules['rules'])) {
                    return $this->__validateField($attribute);
                }
            }
        } elseif ($data && is_array($data)) {
            foreach ($data as $attribute) {
                if (!in_array($this->attributes, $attribute)) {
                    $this->error['msg'] = $this->language->get('error_attribute_doesnt_exists_in_model_mapping'. $attribute);
                    return false;
                }

                if (isset($this->attributes[$attribute]['rules'])) {
                    return $this->__validateField($attribute);
                }
            }
        } else {
            $this->error['msg'] = $this->language->get('error_attribute_null');
            return false;
        }
    }
    ...
    /**
     * 
     * Funcion para mapear todos los campos del modelo
     * esto sera utilizado al momento de generar formularios 
     * utilizando el objeto NtForm
     * 
     * Ejemplo del array con los artibutos
     * 
     * array(
     *  'name'=>'title',
      'label'=>$this->language->get('label_title'),
      'type'=>'text',
      'default'=>'Valor predeterminado',
      'rules'=>array(
            'max_length'=>255, // la longitud maxima permitida para este campo
            'min_length' => 3, // la longitud minima para este campo
            'exact_lentgh' => 20, // la longitud exacta que debe tener este campo
            'decimal'=>1, // solo numeros con decimales permitidos
            'integer'=>1, // solo numeros enteros
            'natural_numbers'=>1, // solo numeros enteros
            'relative_url'=>1, // url sin especificar el protocolo
            'canonical_url'=>1, // url completa incluyendo el protocolo
            'is_array'=>1, // debe ser un array
            'date'=>array( // campo fecha y debe pasarse como parametros el formato de la fecha y el codigo iso de internalizacion
                'format'=>'d/m/Y h:i:s',
                'lang'=>'es'
            ),
            'callback'=>array( // funcion a utilizarse para validar este campo donde debe especificarse la clase php donde se encuentra y el metodo
              'class', // esta clase debe estar mapeada en la aplicacion dentro de /system/validator
              'method'
            ),
            'required'=>1, // campo requerido
            'field_match'=>'input_name', // campo con cual debe coincidir
            'email'=>1, // debe ser un email valido
            'regex_match'=>'regex_pattern' // regex rules personalizadas
        )
      )
     * 
     * @return array field mapping attributes with rules for validations
     */
    protected function __mapAttributes() {
    ...
    public function getErrors() {
        //TODO: mapear todos los tipos de error
        return $this->error;
    }

    protected function __validateField($attribute) {
        if (!isset($this->attributes[$attribute])) {
            $this->error['msg'] = $this->language->get('error_attribute_doesnt_exists_in_model_mapping '. $attribute);
            return false;
        }
        
        $field = trim($this->request->getPost($attribute));
        
        if (isset($this->attributes[$attribute]['rules'])) {
            foreach ($this->attributes[$attribute]['rules'] as $rules => $criteria) {
                switch ($rules) {
                    case 'max_length':
                        if (strlen($field) > $criteria) {
                            $this->error['msg'] = $this->language->get('error_attribute_max_length');
                            return false;
                        }
                        break;
                    case 'min_length':
                        if (strlen($field) < $criteria) {
                            $this->error['msg'] = $this->language->get('error_attribute_min_length');
                            return false;
                        }
                        break;
                    case 'exact_lentgh':
                        if (strlen($field) <> $criteria) {
                            $this->error['msg'] = $this->language->get('error_attribute_exact_length');
                            return false;
                        }
                        break;
                    case 'decimal':
                        if (!$validate->esSoloNumerosConDecimales($field)) {
                            $this->error['msg'] = $this->language->get('error_attribute_decimal');
                            return false;
                        }
                        break;
                    case 'integer':
                        if (!$validate->esSoloNumeros($field)) {
                            $this->error['msg'] = $this->language->get('error_attribute_integer');
                            return false;
                        }
                        break;
                    case 'natural_numbers':
                        if (!$validate->esSoloNumeros($field) && $field >= 0) {
                            $this->error['msg'] = $this->language->get('error_attribute_natural_numbers');
                            return false;
                        }
                        break;
                    case 'relative_url':
                        //TODO: generar regex_match para url relativas
                        break;
                    case 'canonical_url':
                        //TODO: generar regex_match para url canonicas
                        break;
                    case 'is_array':
                        if (!is_array($field)) {
                            $this->error['msg'] = $this->language->get('error_attribute_array');
                            return false;
                        }
                        break;
                    case 'required':
                        if (is_array($field) && count($field) === 0) {
                            $this->error['msg'] = $this->language->get('error_attribute_required');
                            return false;
                        } elseif (empty(trim($field))) {
                            $this->error['msg'] = $this->language->get('error_attribute_required');
                            return false;
                        }
                        break;
                    case 'field_match':
                        if (!$this->request->hasPost($this->attributes[$criteria]) 
                        || $this->request->getPost($this->attributes['field_match']) !==  $field) 
                        {
                            $this->error['msg'] = $this->language->get('error_attribute_field_match');
                            return false;
                        }
                        break;
                    case 'email':
                        if (!$validate->validEmail($field)) {
                            $this->error['msg'] = $this->language->get('error_attribute_email');
                            return false;
                        }
                        break;
                    case 'regex_match':
                        if (!$validate->custom($field,$criteria['pattern'])) {
                            $this->error['msg'] = $this->language->get('error_attribute_regex_match');
                            return false;
                        }
                        break;
                    /*
                    case 'callback':
                        ...
                        $action = new Action($criteria['pattern']);
                        return $controller->dispatch($action, new Action('error/not_found'));
                        break;
                     * 
                     */
                }
    ...
    public function getSeoTitleRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_description` WHERE CHAR_LENGTH(`title`) NOT BETWEEN 8 AND 55");
        $query2 = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_description`");
        return $query->row['total'] * 100 / $query2->row['total'];
    }

    public function getSeoMetaDescripionRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_description` WHERE CHAR_LENGTH(`meta_description`) NOT BETWEEN 8 AND 155");
        $query2 = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_description`");
        return $query->row['total'] * 100 / $query2->row['total'];
    }

    public function getSeoDescriptionRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_description` WHERE CHAR_LENGTH(`description`) < 150");
        $query2 = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_description`");
        return $query->row['total'] * 100 / $query2->row['total'];
    }

    public function getSeoUrlRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post` WHERE post_type = 'page' AND post_id NOT IN (SELECT `object_id` FROM `" . DB_PREFIX . "url_alias` WHERE `object_type` = 'page')");
