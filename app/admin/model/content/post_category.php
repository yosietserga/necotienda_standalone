...
class ModelContentPostCategory extends Model {
    ...
    public function add($data) {
        ...
        $this->setDescriptions($post_category_id, $data['category_description']);
    ...
    /**
     * ModelContentPostCategory::update()
     * 
        ...
        foreach ($shared_tables as $table) {
            $this->db->query("DELETE FROM " . DB_PREFIX . "$table ".
                "WHERE object_id IN ( ".
                    ...
                    "WHERE post_category_id = '" . (int) $category_id . "' ".
                    "OR parent_id = '" . (int) $category_id . "' ".
                ") ".
                "AND object_type = 'post_category'");
        }

        $this->db->query("DELETE FROM " . DB_PREFIX . "post_category ".
            "WHERE post_category_id = '" . (int) $category_id . "' ".
            "OR parent_id = '" . (int) $category_id . "'");
    ...
    public function getPath($category_id) {
        ...
        FROM " . DB_PREFIX . "post_category c 
            LEFT JOIN " . DB_PREFIX . "description cd ON (c.post_category_id = cd.object_id) 
        WHERE c.post_category_id = '" . (int) $category_id . "' 
    ...
    public function getSeoUrlRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_category` WHERE post_category_id NOT IN (SELECT `object_id` FROM `" . DB_PREFIX . "url_alias` WHERE `object_type` = 'post_category') ");
        $query2 = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_category`");
        return $query->row['total'] * 100 / $query2->row['total'];
    }
    ...
    public function getAll($data=null) {
        $cache_prefix = "admin.post_categories";
        ...
        if (!$cached) {
            $sql = "SELECT * FROM " . DB_PREFIX . "post_category t ".
                "LEFT JOIN " . DB_PREFIX . "description td ON (t.post_category_id = td.object_id) ";
    ...
    public function getAllTotal($data=null) {
        $cache_prefix = "admin.post_categories.total";
        $cachedId = $cache_prefix.
            (int)STORE_ID ."_".
            serialize($data).
            $this->config->get('config_language_id') . "." .
        ...
        $cached = $this->cache->get($cachedId, $cache_prefix);
        if (!$cached) {
            $sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "post_category t ".
                "LEFT JOIN " . DB_PREFIX . "description td ON (t.post_category_id = td.object_id) ";
    ...
    private function buildSQLQuery($data, $sort_data = null, $countAsTotal = false) {
        ...
        $criteria[] = " td.object_type = 'post_category' ";
        ...
        if (isset($data['store_id']) && !empty($data['store_id'])) {
            $sql .= "LEFT JOIN " . DB_PREFIX . "post_category_to_store t2s ON (t.post_category_id = t2s.post_category_id) ";
            $criteria[] = " t2s.store_id IN (" . implode(', ', $data['store_id']) . ") ";
        }

        if (isset($data['post_category_id']) && !empty($data['post_category_id'])) {
            $criteria[] = " t.post_category_id IN (" . implode(', ', $data['post_category_id']) . ") ";
        }
        ...
        if (!empty($data['properties'])) {
            $sql .= " LEFT JOIN " . DB_PREFIX . "property pp ON (t.post_category_id = pp.object_id) ";
            foreach ($data['properties'] as $key => $value) {
                ...
                $criteria[] = " pp.object_type = 'post_category' ";
            }
        }
        ...
        if (!$countAsTotal) {
            if (isset($sort_data)) {
                $sql .= " GROUP BY t.post_category_id";
    ...
    public function activate($id) {
        return $this->__activate('post_category', $id);
    }

    public function desactivate($id) {
        return $this->__desactivate('post_category', $id);
    }
    
    public function getDescriptions($id, $language_id=null) {
        return $this->__getDescriptions('post_category', $id, $language_id);
    }

    public function setDescriptions($id, $data) {
        return $this->__setDescriptions('post_category', $id, $data);
    }

    public function getProperty($id, $group, $key) {
        return $this->__getProperty('post_category', $id, $group, $key);
    }

    public function setProperty($id, $group, $key, $value) {
        return $this->__setProperty('post_category', $id, $group, $key, $value);
    }

    public function deleteProperty($id, $group='*', $key='*') {
        return $this->__deleteProperties('post_category', $id, $group, $key);
    }

    public function getAllProperties($id, $group = '*') {
        return $this->__getProperties('post_category', $id, $group);
    }
