...
class ModelContentPostCategory extends Model {
    ...
    public function getSeoTitleRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_category_description` WHERE CHAR_LENGTH(`name`) NOT BETWEEN 8 AND 55");
        $query2 = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_category_description`");
        return $query->row['total'] * 100 / $query2->row['total'];
    }

    public function getSeoMetaDescripionRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_category_description` WHERE CHAR_LENGTH(`meta_description`) NOT BETWEEN 8 AND 155");
        $query2 = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_category_description`");
        return $query->row['total'] * 100 / $query2->row['total'];
    }

    public function getSeoDescriptionRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_category_description` WHERE CHAR_LENGTH(`description`) < 150");
        $query2 = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "post_category_description`");
        return $query->row['total'] * 100 / $query2->row['total'];
    }
