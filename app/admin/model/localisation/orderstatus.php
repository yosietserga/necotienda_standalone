...
class ModelLocalisationOrderStatus extends Model {
	...
	public function update($id, $data) {
		foreach ($data['order_status'] as $language_id => $value) {
			$this->db->query("UPDATE " . DB_PREFIX . "order_status SET ".
            	"name = '" . $this->db->escape($value['name']) . "' ".
            "WHERE order_status_id = '" . (int)$id . "' ".
	...
	public function delete($id) {
        ...
        foreach ($shared_tables as $table) {
            $this->db->query("DELETE FROM " . DB_PREFIX . "$table ".
                ...
                "AND object_type = 'order_status'");
        }

		$this->db->query("DELETE FROM " . DB_PREFIX . "order_status WHERE order_status_id = '" . (int)$id . "'");
	...
	public function getDescriptions($id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_status WHERE order_status_id = '" . (int)$id . "'");
    ...
    public function getAll($data=null) {
        $cache_prefix = "admin.order_statuses";
        ...
        if (!$cached) {
            $sql = "SELECT * FROM " . DB_PREFIX . "order_status t ";
    ...
    public function getAllTotal($data=null) {
        $cache_prefix = "admin.order_statuses.total";
        $cachedId = $cache_prefix.
            (int)STORE_ID ."_".
            serialize($data).
            $this->config->get('config_language_id') . "." .
        ...
        $cached = $this->cache->get($cachedId, $cache_prefix);
        if (!$cached) {
            $sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "order_status t ";
    ...
    private function buildSQLQuery($data, $sort_data = null, $countAsTotal = false) {
        ...
        if (!empty($data['properties'])) {
            $sql .= " LEFT JOIN " . DB_PREFIX . "property tp ON (t.order_status_id = tp.object_id) ";
            foreach ($data['properties'] as $key => $value) {
                ...
                $criteria[] = " tp.object_type = 'order_status' ";
            }
        }
        ...
        if (!$countAsTotal) {
            if (isset($sort_data)) {
                $sql .= " GROUP BY t.order_status_id";
    ...
    public function getProperty($id, $group, $key) {
        return $this->__getProperty('order_status', $id, $group, $key);
    }

    public function setProperty($id, $group, $key, $value) {
        return $this->__setProperty('order_status', $id, $group, $key, $value);
    }

    public function deleteProperty($id, $group='*', $key='*') {
        return $this->__deleteProperties('order_status', $id, $group, $key);
    }

    public function getAllProperties($id, $group = '*') {
        return $this->__getProperties('order_status', $id, $group);
    }
