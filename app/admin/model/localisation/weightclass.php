...
class ModelLocalisationWeightClass extends Model {
	...
	public function add($data) {
        ...
        $this->setDescriptions($weight_class_id, $data['weight_class_description']);
    ...
    public function getAll($data=null) {
        $cache_prefix = "admin.weight_classes";
        ...
        if (!$cached) {
            $sql = "SELECT * FROM " . DB_PREFIX . "weight_class t ".
                "LEFT JOIN " . DB_PREFIX . "description td ON (t.weight_class_id = td.object_id) ";
    ...
    public function getAllTotal($data=null) {
        $cache_prefix = "admin.weight_classes.total";
        $cachedId = $cache_prefix.
            (int)STORE_ID ."_".
            serialize($data).
            $this->config->get('config_language_id') . "." .
        ...
        $cached = $this->cache->get($cachedId, $cache_prefix);
        if (!$cached) {
            $sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "weight_class t ".
                "LEFT JOIN " . DB_PREFIX . "description td ON (t.weight_class_id = td.object_id) ";
    ...
    private function buildSQLQuery($data, $sort_data = null, $countAsTotal = false) {
        ...
        $criteria[] = " td.object_type = 'weightclass' ";
        ...
        if (!empty($data['properties'])) {
            $sql .= " LEFT JOIN " . DB_PREFIX . "property tp ON (t.weight_class_id = tp.object_id) ";
            foreach ($data['properties'] as $key => $value) {
                ...
                $criteria[] = " tp.object_type = 'weightclass' ";
            }
        }
        ...
        if (!$countAsTotal) {
            if (isset($sort_data)) {
                $sql .= " GROUP BY t.weight_class_id";
    ...
    public function getDescriptions($id, $language_id=null) {
        return $this->__getDescriptions('weightclass', $id, $language_id);
    }

    public function setDescriptions($id, $data) {
        return $this->__setDescriptions('weightclass', $id, $data);
    }

    public function getProperty($id, $group, $key) {
        return $this->__getProperty('weightclass', $id, $group, $key);
    }

    public function setProperty($id, $group, $key, $value) {
        return $this->__setProperty('weightclass', $id, $group, $key, $value);
    }

    public function deleteProperty($id, $group='*', $key='*') {
        return $this->__deleteProperties('weightclass', $id, $group, $key);
    }

    public function getAllProperties($id, $group = '*') {
        return $this->__getProperties('weightclass', $id, $group);
    }
