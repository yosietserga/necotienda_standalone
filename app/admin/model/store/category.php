...
class ModelStoreCategory extends Model {
    ...
    public function getAllForList($parent_id = 0, $data = array()) {
        ...
        if ($query->rows) {
            foreach ($query->rows as $key => $value) {
                if (isset($with_child))
                    $childrens = $this->getAllForList($value['category_id'], $data);
                $return[] = array(
    ...
    public function getSeoTitleRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "category_description` WHERE CHAR_LENGTH(`name`) NOT BETWEEN 8 AND 55");
        $query2 = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "category_description`");
        return $query->row['total'] * 100 / $query2->row['total'];
    }

    public function getSeoMetaDescripionRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "category_description` WHERE CHAR_LENGTH(`meta_description`) NOT BETWEEN 8 AND 155");
        $query2 = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "category_description`");
        return $query->row['total'] * 100 / $query2->row['total'];
    }

    public function getSeoDescriptionRating() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "category_description` WHERE CHAR_LENGTH(`description`) < 150");
        $query2 = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "category_description`");
        return $query->row['total'] * 100 / $query2->row['total'];
    }
