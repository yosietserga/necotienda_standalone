...
<div class="box">
  ...
  <div class="content">
        ...
        <div style="float:left"><a onclick="jQuery('.form .on').slideDown();">Mostrar Todos</a>&nbsp;&nbsp;<a onclick="jQuery('.form .on').slideUp();">Ocultar Todos</a></div><div style="float:right"><a onclick="addMemberToAllLists()">Agregar a Todos&nbsp;<img src="view/image/add.png"></a>&nbsp;&nbsp;<a onclick="deleteMemberFromAllLists()">Eliminar a Todos&nbsp;<img src="view/image/delete.png"></a></div>
        <table class="form">
          ...
          <tr class="lists">
            ...
            <td style="float:right"><a onclick="deleteMemberFromList(this)" class="id_<?php echo $b; ?>_d"><img src="view/image/delete.png"></a></td>
          </tr>
            ...
            <td colspan="2">              
          	...
          	<?php if (isset($list[0])) { ?>
                ...
                <?php if (is_array($interes)) { ?>
              		  <div style="width:25%;float:left;"><img src="view/image/vineta.gif">&nbsp;<a style="text-decoration:none" onMouseOver="jQuery(this).css({'text-decoration':'underline'});" onMouseOut="jQuery(this).css({'text-decoration':'none'});" href="<?php echo $categoria.$interes['category_id']; ?>"><?php echo $interes['category_name']; ?></a></div>
                <?php } ?>
               <?php } ?>
             <?php } else { ?>
     ...
     <table class="form">
          ...
          <tr  class="other_lists">
            ...
            <td style="float:right"><a onclick="addMemberToList(this)" class="id_<?php echo $b; ?>"><img src="view/image/add.png"></a></td>
          </tr>
          <tr id="id_<?php echo $b; ?>_i" class="on">
            <td colspan="2"> 
             <h3>Categor&iacute;as</h3>
...
function bounce() {
	jQuery("#cmdTestBounce").fancybox({
				...
				'href'				: 'index.php?route=email/bounce/test&token=<?php echo $token; ?>&bounce_server=' + encodeURIComponent(jQuery("input[name='bounce_server']").val()) 
...
function addMemberToList(e) {
	jQuery.ajax({
		type: 'POST',
		url: 'index.php?route=email/member/addMemberToList&token=<?php echo $_GET["token"]; ?>',
		data: {
...
function addMemberToAllLists() {
	jQuery.ajax({
		type: 'POST',
		url: 'index.php?route=email/member/addMemberToAllLists&token=<?php echo $_GET["token"]; ?>',
		data: {
			'member_id':'<?php echo $_GET["member_id"]; ?>',
			'email':'<?php echo $email; ?>'
			},
...
function deleteMemberFromList(e) {
	jQuery.ajax({
		type: 'POST',
		url: 'index.php?route=email/member/deleteMemberFromList&token=<?php echo $_GET["token"]; ?>',
		data: {
			'member_id':'<?php echo $_GET["member_id"]; ?>',
			'list_id': jQuery("input[name='"+jQuery(e).attr('class')+"']").val()			
			},
...
function deleteMemberFromAllLists() {
	jQuery.ajax({
		type: 'POST',
		url: 'index.php?route=email/member/deleteMemberFromAllLists&token=<?php echo $_GET["token"]; ?>',
