<form id="<?php echo $name; ?>_form">
    ...
    <div class="row">
        ...
        <?php foreach ($layer_sliders as $result) { ?>
            <option value="<?php echo $result['layer_slider_id']; ?>"<?php if ($result['layer_slider_id']==$settings['layer_slider_id']) { ?> selected="selected"<?php } ?>><?php echo $result['name']; ?></option>
        <?php } ?>
        </select>
    </div>
    ...
    <input type="hidden" name="Widgets[<?php echo $name; ?>][settings][route]" value="module/layer_slider" />
</form>
<script>
$(function(){
    $('#widget_layer_sliders<?php echo $name; ?>').chosen();
