...
<form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
    <table id="list">
        ...
        <?php if ($banks) { ?>
            ...
            <tr id="tr_<?php echo $bank['bank_id']; ?>">
                <td><input title="Seleccionar para una acci&oacute;n" type="checkbox" name="selected[]" value="<?php echo $bank['bank_id']; ?>" <?php if ($bank['selected']) { ?>checked="checked"<?php } ?>/></td>
                <td><img src="<?php echo $bank['image']; ?>" alt="<?php echo $bank['name']; ?>" /></td>
                <td><?php echo $bank['name']; ?></td>
                <td><?php echo $bank['date_added']; ?></td>
                <td>
                <?php foreach ($bank['action'] as $action) { ?>
                <?php 
                    if ($action['action'] == "delete") {
                        $jsfunction = "eliminar(". $bank['bank_id'] .")";
                        $href = "";
                        $img_id = "img_del_" . $bank['bank_id'];
                    } elseif ($action['action'] == "edit") {
                        $href = "href='" . $action['href'] ."'";
                        $jsfunction = "";
                        $img_id = "img_edit_" . $bank['bank_id'];
                    } 
                ?>
                <a title="<?php echo $action['text']; ?>" <?php echo $href; ?> onclick="<?php echo $jsfunction; ?>"><img id="img_<?php echo $bank['bank_id']; ?>" src="image/<?php echo $action['img']; ?>" alt="<?php echo $action['text']; ?>" /></a>
                <?php } ?>
                </td>
            </tr>
            <?php } ?>
