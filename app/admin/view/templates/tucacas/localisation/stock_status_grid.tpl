...
<form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
    <table id="list">
        ...
        <?php if ($stock_statuses) { ?>
            ...
            <tr id="tr_<?php echo $stock_status['stock_status_id']; ?>">
                <td><input title="Seleccionar para una acci&oacute;n" type="checkbox" name="selected[]" value="<?php echo $stock_status['stock_status_id']; ?>" <?php if ($stock_status['selected']) { ?>checked="checked"<?php } ?>/></td>
                <td><?php echo $stock_status['name']; ?></td>
                <td>
                <?php foreach ($stock_status['action'] as $action) { ?>
                <?php 
                    if ($action['action'] == "activate") { 
                        $jsfunction = "activate(". $stock_status['stock_status_id'] .")";
                        $href = "";
                    } elseif ($action['action'] == "delete") {
                        $jsfunction = "eliminar(". $stock_status['stock_status_id'] .")";
                        $href = "";
                ...
                ?>
                <a title="<?php echo $action['text']; ?>" <?php echo $href; ?> onclick="<?php echo $jsfunction; ?>"><img id="img_<?php echo $stock_status['stock_status_id']; ?>" src="image/<?php echo $action['img']; ?>" alt="<?php echo $action['text']; ?>" /></a>
                <?php } ?>
                </td>
            </tr>
            <?php } ?>
