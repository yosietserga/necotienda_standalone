...
class ControllerPaymentPayU extends Controller
{
    ...
    {
        $this->load->library('payu/openpayu');

        OpenPayU_Configuration::setMerchantPosId($this->config->get('payu_merchantid'));
        OpenPayU_Configuration::setSignatureKey($this->config->get('payu_signaturekey'));
        OpenPayU_Configuration::setEnvironment('secure');
        OpenPayU_Configuration::setApiVersion ( 2.1 );
        OpenPayU_Configuration::setSender("NecoTienda ver/Plugin ver 2.1.5" );
    ...
    protected function index()
    {
        ...
        if($this->config->get('payu_test_mode')) {
            $this->data['action'] = 'https://sandbox.gateway.payulatam.com/ppp-web-gateway/';
        } else {
        ...
        $this->data['merchantId'] = $this->config->get('payu_merchantid');
        $this->data['ApiKey'] = $this->config->get('payu_api_key');
        $this->data['accountId'] = $this->config->get('payu_accountid');
        $this->data['description'] = $this->language->get('Payment for Order ID #'. $order_id);
        $this->data['referenceCode'] = $order_id;
        $this->data['amount'] = $this->currency->format($order_info['total'], $order_info['currency'], $order_info['value'], FALSE);
        $this->data['tax'] = '0';
        $this->data['taxReturnBase'] = '0';
        $this->data['currency'] = $order_info['currency'];
        $this->data['test'] = $this->config->get('payu_test_mode');
        $this->data['buyerEmail'] = $this->customer->getEmail();

        $this->data['signature'] = md5(
            $this->data['ApiKey'] .'~'.
        ...
        $this->data['responseUrl'] = $Url::createUrl('payment/payu/responsewebcheckout');
        $this->data['confirmationUrl'] = $Url::createUrl('payment/payu/conrimationwebcheckout');
        $this->data['language'] = strtoupper($this->language->get('code'));

        $this->data['shippingAddress'] = $order_info['shipping_address_1'];
        $this->data['shippingCity'] = $order_info['shipping_city'];
        $this->data['shippingCountry'] = $order_info['shipping_iso_code_2'];

        $this->load->model("marketing/newsletter");
        $result = $this->modelNewsletter->getById($this->config->get('payu_newsletter_id'));
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/payment/payu_webcheckout.tpl')) {
            $this->template = $this->config->get('config_template') . '/payment/payu_webcheckout.tpl';
        } else {
            $this->template = 'cuyagua/payment/payu_webcheckout.tpl';
        }

        $this->render();
    }

    public function responsewebcheckout() {}
    public function conrimationwebcheckout() {}
