...
<script data-script="tabs">
    (function () {
        window.deferjQuery(function () {
            var $tabs = $('.tab');
            $('#review').load('<?php echo $Url::createUrl("store/review") ."&object_type=product&object_id=". $product_id; ?>');
            $('#comment').load('<?php echo $Url::createUrl("store/review/comment") ."&object_type=product&object_id=". $product_id; ?>');
    ...
    (function () {
        window.deferjQuery(function () {
            ...
            var openGraphMetaData = [
                ...
                '<meta property="product:price:currency" content="<?php echo $Config->get('config_currency'); ?>" />',
            ];
