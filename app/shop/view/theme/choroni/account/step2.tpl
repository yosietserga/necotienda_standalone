...
<?php if (isset($mostrarError)) echo $mostrarError; ?>
        ...
        <?php } ?>
        <a title="Guardar" class="button" onclick="$('#formWrapper').html('<img src=\'<?php echo HTTP_IMAGE; ?>loader.gif\' alt=\'Cargando...\' />');$.post('<?php echo $save; ?>',$('#form').serialize(),function(data){if (data>0) {$('#formWrapper').load('<?php echo $next; ?>')} })">Guardar</a>
