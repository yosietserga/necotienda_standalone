...
<?php if ($sort) { ?>
    <div class="sort">
      ...
      <?php echo $text_sort; ?>
          ...
          <a class="view_style" onclick="$('#products').html('<img src=\'<?php echo HTTP_IMAGE; ?>nt_loader.gif\' alt=\'Cargando...\' />');$('#products').load('<?php echo str_replace('&', '&amp;', $gridView); ?>')" title="Ver en Miniaturas"  style="background-position:0 -23px">&nbsp;</a>    
          <?php } else { ?>
          <a class="view_style" onclick="$('#products').html('<img src=\'<?php echo HTTP_IMAGE; ?>nt_loader.gif\' alt=\'Cargando...\' />');$('#products').load('<?php echo str_replace('&', '&amp;', $listView); ?>')" title="Ver en Listado">&nbsp;</a>    
          <?php } ?>
    </div> 
<?php } ?>
    ...
    <?php foreach($products as $product) { ?>
        <article>
            <div class="product_preview">
                <a class="thumb" href="<?php echo str_replace('&', '&amp;', $product['href']); ?>" title="<?php echo $product['name']; ?>">
...
function sort(e,a) {
    if (a.length > 0) {
        $('#products').html('<img src=\'<?php echo HTTP_IMAGE; ?>nt_loader.gif\' alt=\'Cargando...\' />');
        $("#products").load(a);
    }
}
</script>
