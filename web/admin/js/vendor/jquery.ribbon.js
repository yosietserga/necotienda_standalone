...
(function($) {
    ...
    $.fn.Ribbon = function(ribbonSettings) {
        ...
        if (isLoaded) {
            ...
            $('.ribbon-list div img[src*="../image/ribbon/arrow_down.png"]').remove();
        }

        $('.orb li:first ul:first').prepend('<img src="view/image/ribbon/menu_top.png" style="margin-left: -10px; margin-top: -22px;" />');
        $('.orb li:first ul:first').append('<img src="view/image/ribbon/menu_bottom.png" style="margin-left: -10px; margin-bottom: -22px;" />');

        $('.ribbon-list div').each(function() { if ($(this).children('ul').length > 0) { $(this).append('<img src="view/image/ribbon/arrow_down.png" style="float: right; margin-top: 5px;" />') } });
